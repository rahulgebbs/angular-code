...
@Component({
  selector: 'app-pcn-report',
  templateUrl: './pcn-report.component.html',
  styleUrls: ['./pcn-report.component.css'],
  providers: [ExcelService, dropDownFields]
})
export class PcnReportComponent implements OnInit {
  ...
  constructor(private excelService: ExcelService, private selectField: dropDownFields, private router: Router, private notification: NotificationService, private simpleservice: SimpleReportService, private fb: FormBuilder, private pcnService: PcnService) {
  ...
  OnFromDateChange(event) {
    ...
    if (event.value > this.ToDate) {
      ...
      this.PCNForm.patchValue({ 'To_Date': todate, 'From_Date': date })
    } else {
      let date = new Date(event.value);
      let todate = new Date()
      this.PCNForm.patchValue({ 'To_Date': todate, 'From_Date': date })
  ...
  OnToDateChange(event) {
    ...
    if (event.value < this.FromDate) {
      ...
      this.PCNForm.patchValue({ 'To_Date': todate })
    }
  }
  ...
  ClientListOnChange(event) {
    ...
    this.PCNForm.patchValue({ 'Employee_Ids': '' });
  ...
  GetSimpleReportData() {
    console.log('this.PCNForm.valid : ', this.PCNForm)
    if (this.PCNForm.valid) {
      this.DisplayError = false;
      this.PCNForm.value.From_Date = moment(this.PCNForm.value.From_Date).format('YYYY-MM-DD')
      this.PCNForm.value.To_Date = moment(this.PCNForm.value.To_Date).format('YYYY-MM-DD')
      // console.log(this.PCNForm.value);

      this.pcnService.GetReportDownload(this.PCNForm.value).pipe(finalize(() => {
        this.EmployeeString = "";
        this.CreateForm();
        //this.resetForm();
      })).subscribe(
  ...
  SetSelectedEmp(event) {
    ...
    this.PCNForm.patchValue({ 'Employee_Ids': selectedId })
  }
  ...
  ExportToExcel() {
    ...
    if (this.only_worked) {
      ...
      if (this.PCNForm.value.From_Time == "" && this.PCNForm.value.Employee_Ids != "") {
        this.PCNForm.patchValue({ 'From_Time': "0:00" })
        this.PCNForm.patchValue({ 'To_Time': "0:00" })
        this.exportFunction()
        this.Employees.forEach(e => {
          e.Is_Selected = false;
        })
  ...
  exportFunction() {
    ...
    if (this.PCNForm.valid) {
      ...
      this.PCNForm.value.To_Date = moment(this.PCNForm.value.To_Date).format('YYYY-MM-DD')
      // console.log(this.PCNForm.value);
      this.pcnService.GetReport(this.PCNForm.value).pipe(finalize(() => {
        this.EmployeeString = "";
        this.CreateForm();
        //this.resetForm();
      })).subscribe(
  ...
  getFromTime() {
    ...
    let startDate = moment(this.PCNForm.value.From_Date).format('YYYY-MM-DD');
    let endDate = moment(this.PCNForm.value.To_Date).format('YYYY-MM-DD');
    let startTime = startDate + " " + this.PCNForm.value.From_Time;
    let endTime = endDate + " " + this.PCNForm.value.To_Time
